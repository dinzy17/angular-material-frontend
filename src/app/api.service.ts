...
//function to save token and user id
  private saveToken(token: string, userId: string, userType: string, username: string, authCode: string, expiryDate: string, billDate: string, planCode: string, emailApiType:string = '', userHeaderDetails: any, mainUserId: any = "", req:any): void {
    this.token = token
    this.userId = userId
    ...
    localStorage.setItem("selectedUserId", "")
    let stayLoggedIn = localStorage.getItem("stayLoggedIn") || sessionStorage.getItem("stayLoggedIn")
    if(!stayLoggedIn) {
...
//function to make request from frontend
      ...
      map((response: TokenResponse) => {
        if (response.data && response.data.token) {
          ...
          if( data.userType === response.data.userType || (data.userType == "AdminWeb" && response.data.userType == "TeamMember")) {
            const { token, userId, userType, username, authCode, expiryDate, billDate, planCode, emailApiType, userHeaderDetails, mainUserId } = response.data
            this.saveToken(token, userId, userType, username, authCode, expiryDate, billDate, planCode, emailApiType, userHeaderDetails, mainUserId, data)
            return response
          } else {
  ...
  public logout(): void {
    ...
    this.removeKeyFromStorage('billDate')
    this.removeKeyFromStorage('planCode')
